const nodeGraphNodeLabels = Object.freeze({
  ...
  quadratureOscillator: "Quadrature Osc",
  ...
  // Jerobeam's own branch never added these to this label map, so every one
  // of these 8 native modules showed its raw camelCase type key ("blubb 1",
  // "keplerBouwkamp 1", etc.) as its node title in the graph instead of a
  // real display name -- a real, pre-existing gap, not something this merge
  // introduced. Names match the ones already used in node-graph-module-store.js.
  blubb: "Jerobeam Blubb",
  boing: "Jerobeam Boing",
  keplerBouwkamp: "Jerobeam Kepler-Bouwkamp",
  mushroom: "Jerobeam Mushroom",
  nyquistShannon: "Jerobeam NyquistShannon",
  radar: "Jerobeam Radar",
  torus: "Jerobeam Torus",
  wirdoSpiral: "Jerobeam WirdoSpiral",
...
const nodeGraphModuleDefinitions = Object.freeze({
  ...
  mushroom: {
    ...
    parameters: [
      ...
      { key: "numMushrooms", label: "Num Mushrooms", defaultValue: "1", min: "-5", mid: "1", max: "5", step: "1" },
      { key: "grow", label: "Grow", defaultValue: "1", min: "0", mid: "0.5", max: "1", step: "0.01" },
      { key: "density", label: "Density", defaultValue: "3", min: "0", mid: "3", max: "100", step: "0.01" },
  ...
  torus: {
    ...
    parameters: [
      { key: "frequency", label: "Frequency", kind: "frequency", defaultValue: "8", min: "0", mid: "20", max: "2000", maxDigits: 5, step: "any" },
      { key: "density", label: "Density", defaultValue: "1", min: "0", mid: "5", max: "50", step: "0.01" },
      { key: "quantizeDensity", label: "Quantize Density", defaultValue: "1", min: "0", mid: "0.5", max: "1", step: "1" },
      { key: "subdensity", label: "Sub Density", defaultValue: "0", min: "0", mid: "1", max: "2", step: "0.01" },
  ...
  radar: {
    ...
    parameters: [
      { key: "frequency", label: "Frequency", kind: "frequency", defaultValue: "1", min: "0", mid: "20", max: "2000", maxDigits: 5, step: "any" },
      { key: "phaseOffset", label: "Phase", defaultValue: "0", min: "0", mid: "0.5", max: "1", step: "0.01" },
      { key: "density", label: "Density", defaultValue: "1", min: "0", mid: "50", max: "100", step: "0.01" },
      { key: "sharp", label: "Sharp", defaultValue: "0", min: "-1", mid: "0", max: "1", step: "0.01" },
  ...
  phosphorGrid2d: {
    bufferedInputs: ["X", "Y"],
    displayHeightGu: 5,
    displayType: "phosphorGrid2d",
    inputs: ["X", "Y"],
    layout: "traceDisplay",
    outputs: [],
    parameters: [],
    visualInputs: [
      { key: "phosphorGrid2dX", label: "X", port: "X" },
      { key: "phosphorGrid2dY", label: "Y", port: "Y" },
    ],
    visualSink: true,
  },
  stereoScope: {
    ...
    displayHeightGu: 5,
    displayType: "stereoScope",
    inputs: ["Left", "Right"],
    layout: "traceDisplay",
    outputs: [],
    parameters: [],
    visualInputs: [
      { key: "stereoScopeLeft", label: "Left", port: "Left" },
      { key: "stereoScopeRight", label: "Right", port: "Right" },
    ],
    visualSink: true,
  },
  badvalMonitor: {
...
function nodeGraphModuleProducesOutputWithoutSignalInput(type) {
  ...
  const inputCapableSources = new Set([
    ...
    "output",
    "reverbEffect",
    // Jerobeam's own branch never registered these 8 native modules here --
    // they all have a "Reset" input port (so they fail the zero-input-ports
    // shortcut above) but produce their own X/Y output with no signal input
    // otherwise, same shape as lorenzAttractor/fbPolyBlepOsc/etc. above.
    // Without this, compileNodeGraphExecutionPlan() rejected every one of
    // them with "unsupported source <id>" and live audio silently never
    // started (plan-blocked) whenever one was wired into a patch.
    "blubb",
    "boing",
    "keplerBouwkamp",
    "mushroom",
